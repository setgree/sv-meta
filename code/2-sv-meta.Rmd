...
> In our database we find this relationship between effect sizes and standard errors to be weak and non-significant: $\beta$ = 0.16 (SE = 0.176), p = 0.362.

```{r publication_bias_overall}
sum_lm(dat, d, se_d, coefs_only = T)
```

> Second, the relationship between SE and $\Delta$ remains non-significant within randomized (N = 105, $\beta$ = 0.22, SE = 0.248, p = 0.375) and quasi-experimental (N = 100, $\beta$ = -0.145, SE = 0.407, p = 0.722) designs, providing further evidence to the lack of publication bias within the literature. However, we do find a significant relationship within observational designs (N = 96, $\beta$ = 2.26, SE = 0.642, p $<$ 0.001), suggesting some publication bias within these studies. 
...
> Our third check for publication bias is to separate the literature into studies published in peer-reviewed journals (N = 189) and those that were not (N = 118). 
...
> Within published studies, the relationship between standard errors and $\Delta$ is tiny and insignificant ($\beta$ = 0.021, SE 0.199, p = 0.914); while the association between the two variables is larger in unpublished studies (N = 118, $\beta$  = 0.591, SE = 0.361), the relationship is still statistically non-significant (p = 0.106).
...
> Overall we find that behaviors are much more resistant than ideas to change. The overall effect size for behaviors is $\Delta$ = 0.071 (SE = 0.022), p = .0015; whereas the overall effect size for idea-based outcomes is $\Delta$ = 0.366 (SE = .031), p < 0.0001.
...
> Overall, 22 studies measured whether bystander interventions increase bystander behaviors, which they do to a modest extent $\Delta$ = 0.154 (SE = 0.056), p = 0.011. In our database, 20 bystander studies measure perpetration or victimization, the ultimate goal of these interventions. We did not find that bystander interventions meaningfully change rates of perpetration  ($\Delta$ = 0.019, SE = 0.019, p = 0.329) or victimization ($\Delta$ =-0.009, SE = 0.041,  p = 0.835).
...
> In this subset of studies, the pooled behavior change effect is $\Delta$ = 0.083 (SE = 0.029), p = 0.006, while the pooled idea change effect is $\Delta$ = 0.29 (SE = 0.039), p < 0.0001. In other words, the idea change effects are slightly lower than the overall effect we obtained when looking at the full database ($\Delta$ = 0.368), while the behavior change effects are slightly larger than the overall effect ($\Delta$ = 0.071).
...
> In other words, the idea change effects are slightly lower than the overall effect we obtained when looking at the full database ($\Delta$ = 0.366, SE = 0.031), while the behavior change effects are slightly larger than the overall effect ($\Delta$ = 0.071, 0.022).
...
> Unfortunately, in these studies, we find a small, statistically non-significant correlation of *r* (60) = 0.136, p = 0.293 between changes in ideas and changes in behaviors
...
#' outcomes pre-post. (Things like this issue are why we get an N of 
#' 301 when we divide the studies into design and then count unique_study_ids, 
...
> The relationship remains small and non-significant when looking solely at randomized evaluations (*r* = 0.138, p = 0.48). 
...
**optional: record computational environment**
Here's how we recorded our computational environoment as a Dockerfile. See the [Rocker project](https://rocker-project.org/) for more on this approach to reproducibility.
```{r write_dockerfile, eval = T}
