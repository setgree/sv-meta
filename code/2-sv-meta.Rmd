...
```
> Unfortunately, in these studies, we find a small, statistically non-significant correlation of $\rho$ (58) = 0.112, p = 0.928 between changes in ideas and changes in behaviors.
...
#' note on warning message about removed rows -- that's Peterson 2014, which, 
#' for some variables, gave us enough info to make 'quasi-experimental' comparisons
#' one of two treatments vs a non-randomly selected control) but didn't for IRMA
#' so the attitude variable is effectively a pre-post observation even though the study is 
#' quasi-experimental. That was hard to square with the way we made the figure.
```
