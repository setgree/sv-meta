...
Correlation between ideas and behaviors -- standard errors of estimates
...
### earliest behavioral outcomes?

The first decade of studies overwhelmingly measured ideas rather than behaviors (63 ideas-based outcomes compared to 13 behavioral measurements).

```{r first_decade_outcome type}
dat |> 
...
```
The first behavioral outcomes in our dataset were recorded in 1991

```{r earliest_behavioral_measure}
dat |> 
  filter(scale_type == 'behavior') |> 
  group_by(unique_paper_id) |>
  arrange(year) |> 
  head() |> 
  select(author, year, paper_title, behavior_type)

```

# Figures we didn't end up including
...
This is figure 3 in the main text. This is a slightly different way to code it, including for completeness. 
...
Here was a rough idea for a 'figure 1' analogous to the D/SE plot in the contact hypothesis paper. But it turned out to not be very informative. 
...
### Correlation scatterplot alternatives
Here's a much messier way of slicing the data to get at the relationship between attitudinal and behavioral change, and then another attempt at showing the relationship:
...
Now just the RCTs, and with study labels:
...
dat_for_forest_plot |> group_by(unique_paper_id) |>
  summarise(mean_d_attitude = mean(d[scale_type == "ideas"]),
