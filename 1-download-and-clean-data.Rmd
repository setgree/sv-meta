...
#' now fix the Ns and anticipated directions and such
...
# plus the redline thing isn't workign consistently, some studies are coming through as
# is_red = null
...
na_effs <- raw_dat %>% filter(is.na(eff_type)) # no, because 
# on 06/18/2021 I addressed all of the above studies and either 
...
raw_dat <- raw_dat %>%
  mutate(study_design = 
           case_when(!is.na(n_t_group) &
                       study_design == 'rct' & 
                       between(n_t_group + n_c_group, 3, 10) ~ 'pragmatic rct',
                     !is.na(n_t_group) &
                       study_design == 'rct' &
...
There's a lot more cleaning that could be done -- All marked with TODOs in the above text -- , but if I'm not mistaken, we can go ahead with the core parts of the analysis and return to this.
