...
if (nrow(filter(raw_dat, is_red == "Loading...")) > 0) {
  stop("Error loading red rows, rerun script")
} 
...
raw_dat <- raw_dat %>%
  ...
  select(-c(coder, page_number, comments, # drop team-internal stuff 
            d, var_d, se_d, checked, checker_comment, first_line))
...
# CHECK: are there any ctrl_sds that are NA when that's going to break the calculator?
raw_dat %>% filter(is.na(ctrl_sd)) %>%
...
which(!is.na(raw_dat$ctrl_sd) & is.na(raw_dat$eff_type)) 
# 0 as of 06/18/21
```
...
# any NAs in publication_type?
raw_dat %>% 
  filter(is.na(publication_type)) %>%   
  select(author, year, intervention_name, 
         scale_name, delay, unique_study_id, unique_paper_id)
# TODO: double-check mcleod  1997 publication type
...
raw_dat <- raw_dat %>%
  mutate(condition_gender = str_replace_all(condition_gender, 
                                           c("\\bboys\\b" = "male", 
                                             "\\bmen\\b" = "male", 
                                             "\\bwomen\\b" = "female", 
                                             "\\bgirls\\b" = "female")))
...
## TODO: CHECK EVERYTHING AGAINST coding instructions.docx
```
