...
### Start with turning `-` into NA (chatGPT wrote this)
raw_dat[, c('n_t_pre', 'n_t_post', 'n_c_pre', 
  ...
  lapply(raw_dat[, c('n_t_pre', 'n_t_post', 'n_c_pre', 'n_c_post', 'n_t_group',
                     'n_c_group')], function(x) gsub(pattern = '-', 
                                                     replacement = NA, 
                                                     x = x))

# not given/ dashes/etc. -- make all NA
...
table(Ns$n_t_pre)
raw_dat_test <- raw_dat %>%
  mutate(across(.cols = c(starts_with(c('n_t_', 'n_c_'))), 
                .fns = ~ as.numeric(.x, na.rm = TRUE), .names = "new_{col}")) %>% 
  mutate(u_s_d = as.numeric(u_s_d), .name_repair = "minimal") %>%
  mutate(anticipated_direction = as.numeric(anticipated_direction), .name_repair = "minimal")
# TODO: revisit these warnings

# TODO: revisit this next one, I think it stopped working because of a 
# dplyr update (argh)
# na_vals <- raw_dat %>% filter(is.na(anticipated_direction) |is.na(across(.cols = c(starts_with('n_'))))) %>%
#     select(author, year, paper_title, eff_type, u_s_d, ctrl_sd,  anticipated_direction, n_t_pre, n_c_pre,
#          n_t_post, n_c_post, n_t_group, n_c_group,
#          n_conditions, n_items,
#          unique_paper_id, intervention_name,
#          study_design, scale_name)
# march 2022 update, they were missing data in the n_items column, which I fixed
rm(na_vals, na_dat, Ns)
