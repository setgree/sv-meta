...
# what if we make a scatterplot
dat_for_scatterplot <- dat_for_forest_plot %>%
  mutate(study_names = sub("^(\\w+\\s\\w+).*", "\\1", study_names),
         attitudes_or_behaviors = ifelse(scale_type == "attitudes", "attitudes", "behaviors"))

ggplot(dat_for_scatterplot, aes(x = d, y = se_d, color = scale_type, label = study_names)) +
  geom_point(size = 2) +
#  geom_text(hjust = 0.5, vjust = -1.5) +
  scale_color_manual(values = c("#F8766D", "#00BFC4")) +
  theme_minimal()
# to do: figure out the labeling
