...
# NOTE ON SUM_LM: 
# `sum_lm(dat, d, se_d, coef_only = T)` = `summary(lm(formula = d ~ se_d, data =  dat))$coefficients`
```
...
dat %>% 
  split(.$study_design) %>% 
  map(.x = ., .f = ~dplot(sa_data = ., dot_size = 2, condense = F, dot_informative = T))
...
behavioral_data <- dat %>% 
  ...
  behavior_type = as.factor(case_when(
    ...
    TRUE ~ 'other')))
table(behavioral_data$behavior_type)
...
robust(rma(yi = d, vi = var_d, mods = behavior_type, data = behavioral_rcts), cluster = behavioral_rcts$unique_study_id)
