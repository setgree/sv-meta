...
### just for attitudes?
dat %>% filter(behavior_type == 'attitude') %>% sum_lm(d, delay)


## What's the longest attitude delay measurement?
dat %>% filter(behavior_type == 'attitude') %>% pull(delay) %>% max()
...
dat_for_forest_plot <- raw_dat %>% 
  ...
  select(index, study_names, d, se_d, 
         scale_type, unique_paper_id, behavior_type)
...
p <- dat_for_forest_plot %>%
  ...
  scale_y_continuous(name = "", , breaks=1:length(dat_for_forest_plot$study_names),  , trans = "reverse",
