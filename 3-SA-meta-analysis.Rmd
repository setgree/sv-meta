...
robust(x = rma(yi = d, vi = var_d, data = dat), cluster = dat$unique_study_id)
...
```{r}
# new variable for whether a study has attitudes, behaviors, or both
dat <- dat %>% 
...
has_both_two <- has_both %>% 
  pivot_wider(names_from = scale_type, 
              values_from = c(d, var_d, se_d)) %>%
  select(author, year, paper_title, unique_study_id,
         unique_paper_id,
         d_attitudes, d_behavior) 

cor.test(x = has_both_two$d_attitudes, 
         y = has_both_two$d_behavior)
...
# let's figure out a clever way to plot this
has_both_rcts %>%
  mutate( mean_att_d )
  ggplot(mapping = aes(x = mean_d[has_both$scale_type == 'attitudes'],
                       y = mean_d[has_both$scale_type == 'behavior'])) +
  geom_point()
