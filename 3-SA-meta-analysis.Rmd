...
# new variable for whether a study has attitudes,
# behaviors, or both (used later)
# dat <- dat %>% # why did I leave this blank
...
# map is beautiful once you get the hang of it
...
# declines by about half
dat %>% 
  split(as.factor(dat$delay > 0)) %>%
  map(.f = ~summary(lm(.x$d ~ .x$scale_type)))
...
rct_behavioral %>%
  split(rct_behavioral$type_of_behavior) %>%
  map(.f = ~robust(rma(yi = .x$d, vi = .x$var_d), 
                   cluster = .x$unique_study_id))
# still about 0.1
# TODO: these _all_ have delays. maybe something like delay > 1 month?
rct_perp <- rct_behavioral %>% filter(type_of_behavior == 'perpetration')
